...
if app_mode == "基础单摆模拟":
    ...
    with st.expander("参数对单摆运动的科学效应"):
        ...
        - **对周期的影响**: $T = 2\\pi\\sqrt{L/g}$。增加摆长会按照平方根关系增加周期。
        - **对频率的影响**: $f = \\frac{1}{2\\pi}\\sqrt{g/L}$，增加摆长会降低振动频率。
        ...
        - **对周期的影响**: $T = 2\\pi\\sqrt{L/g}$。增加重力会按照反平方根关系减小周期。
        - **对恢复力的影响**: $F_{restore} = mg\\sin\\theta$。增加g会增加恢复力，导致更快的振荡。
        - **对自然频率的影响**: $\\omega_0 = \\sqrt{g/L}$，增加重力会增加自然频率。
