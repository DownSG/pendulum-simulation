...
with st.sidebar.expander("参数影响详解", expanded=False):
    ...
    **1. 摆长 (L)**
    ...
    - **影响能量**: 势能 $U = mgL(1-\cos\theta)$。摆长增加，相同角度下的最大势能增加。
    - **影响运动**: 摆长增加，线速度增加，但角速度减小。
    
    **2. 质量 (m)**
    - **对周期无影响**: 在理想单摆中，周期与质量无关。
    - **影响能量**: 动能 $K = \frac{1}{2}mL^2\dot{\theta}^2$。质量增加，能量增加。
    - **影响阻尼效应**: 质量越大，相同阻尼系数下受空气阻力影响越小。
    
    **3. 重力加速度 (g)**
    - **影响周期**: $T = 2\pi \sqrt{L/g}$。重力加速度增加，周期减小，呈反比例平方根关系。
    - **影响角加速度**: $\ddot{\theta} = -\frac{g}{L}\sin\theta - \frac{b}{m}\dot{\theta}$。g增加，角加速度增加。
    - **影响能量转换率**: g增大，势能与动能的转换更快。
    
    **4. 阻尼系数 (b)**
    - **影响能量损耗**: $\frac{dE}{dt} = -b\dot{\theta}^2$。阻尼系数增加，能量损耗更快。
    - **影响振幅衰减**: 阻尼系数增加，振幅衰减更快，呈指数关系。
    - **影响周期微扰**: 大阻尼会略微增加周期 $T_{damped} = T_{undamped}\sqrt{1-\zeta^2}$，其中$\zeta$为阻尼比。
    
    **5. 初始角度 ($\\theta_0$)**
    - **影响非线性效应**: 小角度下($<10°$)，$\sin\theta \approx \theta$，可简化为简谐运动；大角度引入非线性效应。
    - **影响周期修正**: 大角度下，实际周期为 $T = T_0(1 + \frac{1}{16}\sin^2(\frac{\theta_0}{2}) + ...)$，其中$T_0$为小角近似周期。
    - **影响最大能量**: 初始角度增加，系统总能量增加，$E_{total} = mgL(1-\cos\theta_0)$。
    
    **6. 模拟时长**
    - 影响观察到的周期数量和阻尼效应累积。
    - 时间越长，能观察到的能量损耗越明显。
    
    *所有影响基于精确的物理模型，包括非线性单摆微分方程：$\ddot{\theta} + \frac{b}{m}\dot{\theta} + \frac{g}{L}\sin\theta = 0$*
    """)
...
if app_mode == "单摆基本模拟":
    ...
    with st.expander("单摆物理模型详解", expanded=False):
        ...
        - **考虑阻尼的周期**：$T = \\frac{2\\pi}{\\omega_d} = \\frac{2\\pi}{\\omega_0\\sqrt{1-\\zeta^2}}$，其中$\\omega_0 = \\sqrt{\\frac{g}{L}}$，$\\zeta = \\frac{b}{2m\\omega_0}$
        ...
        本模拟使用了Runge-Kutta 4-5阶方法（RK45）求解微分方程，这是一种高精度的自适应步长积分算法，能够准确处理非线性系统的演化。
        """)
